...
def to_ll(x,y):
        transformer = Transformer.from_crs("EPSG:32717","EPSG:4326")
...
def fault_CMT(lat,lon,depth,M,strikeF,dipF,nstr,ndip):
	...
	fault_dep1 = numpy.zeros([nstr*ndip, 1])
	fault_dep2 = numpy.zeros([nstr*ndip, 1])
	fault_dep3 = numpy.zeros([nstr*ndip, 1])
	fault_dep4 = numpy.zeros([nstr*ndip, 1])
	...
	for j in range (0, ndip):
		for i in range (0, nstr):
			...
			(fault_lat[k], fault_lon[k]) = to_ll(fault_X*1000,fault_Y*1000)

			(fault_lat1[k], fault_lon1[k]) = to_ll(fault_X1*1000,fault_Y1*1000) 
			(fault_lat2[k], fault_lon2[k]) = to_ll(fault_X2*1000,fault_Y2*1000) 
			(fault_lat3[k], fault_lon3[k]) = to_ll(fault_X3*1000,fault_Y3*1000)
			(fault_lat4[k], fault_lon4[k]) = to_ll(fault_X4*1000,fault_Y4*1000) 

			fault_dep1[k] = z0 + (j)*DWID*math.sin(dipF*math.pi/180)
			fault_dep2[k] = z0 + (j)*DWID*math.sin(dipF*math.pi/180)
			fault_dep3[k] = z0 + (j+1)*DWID*math.sin(dipF*math.pi/180)
			fault_dep4[k] = z0 + (j+1)*DWID*math.sin(dipF*math.pi/180)
			k = k+1

	return (fault_lon, fault_lat, fault_alt, strike, dip, DLEN*numpy.ones([nstr*ndip,1]), DWID*numpy.ones([nstr*ndip,1]), fault_lon1, fault_lat1,fault_lon2, fault_lat2, fault_lon3, fault_lat3, fault_lon4, fault_lat4, fault_dep1, fault_dep2, fault_dep3, fault_dep4)


#A = fault_CMT(0,-80,10,7.5,60,80,10,5)

#print(A[15],A[16],A[17],A[18])		
